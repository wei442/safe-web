<template>
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="orgForm" size="small" style="float: left;">
				<el-form-item label="机构名称">
					<el-input v-model.trim="orgForm.orgName" placeholder="请输入机构名称" clearable></el-input>
				</el-form-item>
				<el-form-item>
		...
		<el-table :data="tableData" border fit highlight-current-row v-loading="listLoading" stripe style="width:100%;" size="medium">
			...
			<el-table-column prop="parentOrgName" label="父机构名称" header-align="center" align="center"></el-table-column>
			<el-table-column prop="orgStatus" label="机构状态" :formatter="formatOrgStatus" header-align="center" align="center"></el-table-column>
		...
		<el-dialog title="查看" :visible.sync="showDialogVisible">
			<el-form :model="showForm" label-width="80px">
				<el-form-item label="机构名称">{{ showForm.orgName }}</el-form-item>
				<el-form-item label="机构状态">{{ showForm.orgStatus == 0 ? '无效' : showForm.orgStatus == 1 ? '有效': '' }}</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			handleSizeChange(val) {
	      	...
	      	//机构内容显示转换
	      	formatOrgStatus: function (row, column) {
				return row.orgStatus == 0 ? '无效' : row.orgStatus == 1 ? '有效': '';
			},
			//搜索
	        search: function(){
	            this.loadData();
			...
			loadData: function() {
				...
				let _this = this;
				axios.post('/org/getListByPage', params).then(function(response) {
